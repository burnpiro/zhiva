...
export class MetadataProvider {
  datasets: any;
  studies: any;
  imageIdToUIDs: Map<string, InstanceUIDs>;
  ...
  constructor() {
    ...
    this.imageIdToUIDs = new Map<string, InstanceUIDs>();
    this.datasets = {};
  }

  async addInstance(dicomJSONDatasetOrP10ArrayBuffer: any, options: any = {}) {
  ...
  addImageIdToUIDs(imageId: string, uids: InstanceUIDs) {
  ...
  _getAndCacheStudyDataset(StudyInstanceUID: string, dataset: any) {
    if (!this.datasets[StudyInstanceUID]) {
      this.datasets[StudyInstanceUID] = dataset;
    }
  }

  getStudyDataset(StudyInstanceUID: string) {
    return this.datasets[StudyInstanceUID];
  }

  _getAndCacheStudy(StudyInstanceUID: string) {
  ...
  _getAndCacheSeriesFromStudy(study: any, SeriesInstanceUID: string) {
  ...
  _getAndCacheInstanceFromStudy(series: any, SOPInstanceUID: string) {
  ...
  async _checkBulkDataAndInlineBinaries(instance: any, server: any) {
  ...
  getTag(query: string, imageId: string, options: any) {
    return this.get(query, imageId, options);
  }

  getInstance(imageId: string) {
    return this.get(INSTANCE, imageId);
  }

  getTagFromInstance(
    naturalizedTagOrWADOImageLoaderTag: any,
    instance: any,
  ...
  _getUIDsFromImageID(imageId: string) {
